 ...
 <script type="text/javascript">
        ...
        $(function () {
            ...
            datagrid = $("#dd").datagrid({
                url: 'userrole_findall.action', //请求的数据源
                ...
                idField: 'urid', //主键
                columns: [[//显示的列
                {field: 'urid', title: '桥梁ID', width: 100, sortable: true, 
                	checkbox:true,
                	},
                 { field: 'id', title: '用户ID', width: 100, sortable: true,
                  ...
                  { field: 'rid', title: '角色ID', width: 100, sortable: true,
                	 editor: { type: 'validatebox', options: { required: true}}
                	}
                   ]],
                	   ...
                	   $.messager.confirm("提示","你确定要删除吗?",function (r){
                		...
                		if(r){
                			for(var i=0;i<rows.length;i++){
                				ids.push(rows[i].urid);
                				alert(rows[i].id);
                				$.ajax({
                    				url:"userrole_deleteur.action",
                    				type:'post',
                    				data:{urid:rows[i].urid},
               ...
               { text:'赋予等级', iconCls:'icon-add',handler:function(){
                   ...
                   var url="userrole_addorup.action?status=add";
			  ...
			  {	text: '弹窗修改', iconCls: 'icon-edit', handler: function(){
				  ...
				  var url="userrole_addorup.action?status=up&orderid="+row.orderid;
				  $('#div1').dialog('refresh',url);
				  div1.dialog('open');
    ...
    //..........1,   
               //增加修改AJAX,
                 onAfterEdit:function(rowIndex, rowData, changes) {
                     ...
                     $.ajax({
                  		//async : false,
                  		url:"userrole_saveorupdate.action",
                  		type:"POST",
                  		data:{
                  			"ur.urid":rowData.urid,
                  			"ur.id":rowData.id,
                  			"ur.rid":rowData.rid
                  		},
                  		success:function(data){
